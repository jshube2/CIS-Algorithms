...
'''
Parses through the ct-fiducials file and returns the CT-Fiducial array.
'''
def read_ct_fiducials(filename):

    fiducials = []
    ct_fiducials = open(filename, 'r')
    first_line = ct_fiducials.readline().split(",")
    for x in first_line:
        x.strip()

    num_pts = int(first_line[0])

    # iterate through the ct fiducial points
    for i in range(0, num_pts):
        current_line = ct_fiducials.readline().split(",")
        for x in current_line:
            x.strip()
